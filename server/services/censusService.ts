...
export async function getExpandedDemographics(state: string, city: string): Promise<ExpandedDemographics | null> {
  try {
    ...
    if (!locationMatch) {
      console.warn(`Location not found: ${city}, ${state}`);
      return null;
    }
    ...
    // Fetch demographic data using the FIPS codes
...
export async function getIncomeEmploymentData(state: string, city: string): Promise<IncomeEmploymentData | null> {
  try {
    ...
    // Fetch income and employment data using the FIPS codes
...
export async function getExpandedHousingData(state: string, city: string): Promise<ExpandedHousingData | null> {
  try {
    ...
    // Fetch housing data using the FIPS codes
...
export async function getEducationData(state: string, city: string): Promise<EducationData | null> {
  try {
    ...
    // Fetch education data using the FIPS codes
    const educationUrl = `${ACS_5YEAR_BASE}?get=B15003_001E,B15003_017E,B15003_018E,B15003_019E,B15003_020E,B15003_021E,B15003_022E,B15003_023E,B15003_024E,B15003_025E,B14001_002E,B14001_003E,B14001_004E,B14001_005E,B14001_006E,B14001_007E,B14001_008E,B15003_002E,B15003_003E,B15003_004E,B15003_005E,B15003_006E,B15003_007E,B15003_008E,B15003_009E,B15003_010E,B15003_011E,B15003_012E,B15003_013E,B15003_014E,B15003_015E,B15003_016E&for=place:${placeCode}&in=state:${stateCode}&key=${CENSUS_API_KEY}`;
    ...
    const totalPopulation25Plus = parseInt(row[0]);
    ...
    // High school or equivalent (row[1])
    // Some college, no degree (row[2])
    // Associate's degree (row[3])
    // Bachelor's degree (row[4])
    // Master's degree (row[5])
    // Professional degree (row[6])
    // Doctorate degree (row[7])
    highSchoolCount = parseInt(row[1]) + parseInt(row[2]) + parseInt(row[3]) + parseInt(row[4]) + parseInt(row[5]) + parseInt(row[6]) + parseInt(row[7]) + parseInt(row[8]);
    bachelorsCount = parseInt(row[4]) + parseInt(row[5]) + parseInt(row[6]) + parseInt(row[7]) + parseInt(row[8]);
    graduateCount = parseInt(row[5]) + parseInt(row[6]) + parseInt(row[7]) + parseInt(row[8]);
    
    // Calculate percentages for educational attainment
    const highSchoolOrHigher = Math.round((highSchoolCount / totalPopulation25Plus) * 100);
    const bachelorsOrHigher = Math.round((bachelorsCount / totalPopulation25Plus) * 100);
    const graduateOrProfessional = Math.round((graduateCount / totalPopulation25Plus) * 100);
    
    // School enrollment
    const totalEnrollment = parseInt(row[9]);
    ...
    // Calculate percentages for school enrollment
    const schoolEnrollment = {
      preschool: Math.round((preschool / totalEnrollment) * 100),
      kindergarten: Math.round((kindergarten / totalEnrollment) * 100),
      elementary: Math.round((elementary / totalEnrollment) * 100),
      highSchool: Math.round((highSchool / totalEnrollment) * 100),
      college: Math.round((college / totalEnrollment) * 100)
    };
...
export async function getCommuteData(state: string, city: string): Promise<CommuteData | null> {
  try {
    ...
    // Fetch commute data using the FIPS codes
    const commuteUrl = `${ACS_5YEAR_BASE}?get=B08303_001E,B08303_013E,B08301_001E,B08301_003E,B08301_004E,B08301_010E,B08301_019E,B08301_021E,B08302_002E,B08302_003E,B08302_004E,B08302_005E,B08302_006E,B08302_007E,B08302_008E,B08302_009E,B08302_010E,B08302_011E,B08302_012E,B08302_013E,B08302_014E,B08302_015E&for=place:${placeCode}&in=state:${stateCode}&key=${CENSUS_API_KEY}`;
    ...
    // Parse commute values
    const meanTravelTimeToWork = parseFloat(row[1]);
    
    // Commute type
    const totalCommuters = parseInt(row[2]);
    const driveAlone = parseInt(row[3]);
    const carpool = parseInt(row[4]);
    const publicTransit = parseInt(row[5]);
    const walk = parseInt(row[6]);
    const workFromHome = parseInt(row[7]);
    const other = totalCommuters - (driveAlone + carpool + publicTransit + walk + workFromHome);
    
    // Calculate percentages for commute type
    const commuteType = {
    ...
    // Before 5:00 a.m. (row[8])
    // 5:00 a.m. to 5:29 a.m. (row[9])
    // 5:30 a.m. to 5:59 a.m. (row[10])
    // 6:00 a.m. to 6:29 a.m. (row[11])
    // 6:30 a.m. to 6:59 a.m. (row[12])
    before7am = parseInt(row[8]) + parseInt(row[9]) + parseInt(row[10]) + parseInt(row[11]) + parseInt(row[12]);
    
    // 7:00 a.m. to 7:29 a.m. (row[13])
    // 7:30 a.m. to 7:59 a.m. (row[14])
    from7to8am = parseInt(row[13]) + parseInt(row[14]);
    
    // 8:00 a.m. to 8:29 a.m. (row[15])
    // 8:30 a.m. to 8:59 a.m. (row[16])
    from8to9am = parseInt(row[15]) + parseInt(row[16]);
    
    // 9:00 a.m. to 9:59 a.m. (row[17])
    // 10:00 a.m. to 10:59 a.m. (row[18])
    // 11:00 a.m. to 11:59 a.m. (row[19])
    // 12:00 p.m. to 3:59 p.m. (row[20])
    // 4:00 p.m. to 11:59 p.m. (row[21])
    after9am = parseInt(row[17]) + parseInt(row[18]) + parseInt(row[19]) + parseInt(row[20]) + parseInt(row[21]);
    
    // Calculate percentages for departure time
    const departureTimeTotal = before7am + from7to8am + from8to9am + after9am;
    const departureTime = {
      before7am: Math.round((before7am / departureTimeTotal) * 100),
      from7to8am: Math.round((from7to8am / departureTimeTotal) * 100),
      from8to9am: Math.round((from8to9am / departureTimeTotal) * 100),
      after9am: Math.round((after9am / departureTimeTotal) * 100)
    };
...
export async function getAllExpandedCensusData(state: string, city: string): Promise<ExpandedCensusData | null> {
  try {
    const demographics = await getExpandedDemographics(state, city);
    const incomeEmployment = await getIncomeEmploymentData(state, city);
    const housing = await getExpandedHousingData(state, city);
    const education = await getEducationData(state, city);
    const commute = await getCommuteData(state, city);
    
    if (!demographics || !incomeEmployment || !housing || !education || !commute) {
      console.warn(`Some census data could not be fetched for ${city}, ${state}`);
    }
